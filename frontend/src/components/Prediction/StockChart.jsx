...
const StockChart = () => {
    ...
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState('');
    const [railwayPrediction, setRailwayPrediction] = useState(null);  // New state for Railway API prediction
    const companies = ['AAPL', 'BLK', 'GOOG', 'MSFT', 'UBER'];
    ...
    const fetchStockData = async (company) => {
        ...
        try {
            ...
            setRailwayPrediction(railwayResponse.data);  // Save the prediction response to state
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
            ...
            <div className="mt-6 p-4 bg-white shadow-md rounded-lg w-full md:w-3/4 lg:w-1/2 text-center">
                            <h3 className="text-xl font-bold text-gray-800">Predictions from Railway API:</h3>
                            <pre className="text-sm text-gray-700 mt-2">{JSON.stringify(railwayPrediction, null, 2)}</pre>
                        </div>
            {loading ? (
                <p className="text-sm text-gray-500">Loading...</p>
            ) : error ? (
